...
jQuery(document).ready(function($){
	...
	// Blog Image Overlay
	$('.blog-wrap .post .entry-image a').append( '<div class="entry-overlay"><i class="icon-titanium-link"></i></div>' );
	$('.blog-single .post .entry-image a').append( '<div class="entry-overlay"><i class="icon-titanium-search"></i></div>' );
	...
	function flexsliderLoad() {
   	...
   	function zoomingSlider() {
		if ($('.titanium_zooming_slider').length) {
			$('.titanium_zooming_slider').each(function() {
				var $slider = $(this);
				$slider.find('.titanium_zooming_slider_item').eq(0).addClass('active');
				$slider.flexslider({
					animation: "slide",
					animationSpeed: 400,
					useCSS: true,
					...
					directionNav: true,
					start: function(slider) {
						$slider.find('.titanium_zooming_slider_item').css({ 'opacity': '1', 'filter': 'alpha(opacity=100)' });
						var $titles = $slider.find('li:not(.clone) .titanium_zooming_slider_item .image_wrapper .titanium_zooming_slider_title');
						...
						$control_links.each(function() {
							$(this).html('');
							$('<div class="titanium_zooming_slider_ghost'+($(this).parent().is(':first-child') ? ' shown' : '')+'"></div><div class="nav_title"><h6>' + $titles.eq($control_links.index($(this))).html() + '</h6></div>').insertAfter($(this)).click(function() {
						...
						$(slider).find('.slides').mousedown(function(e) {
							...
							var mousestart = parseInt($(this).attr('data-mousestart'),10);
							var mouseend = e.screenX;
							$(this).parents('.titanium_zooming_slider').find('.flex-direction-nav .flex-' + (mousestart - mouseend >= 0 ? 'next' : 'prev')).trigger('click');
						});
					},
					before: function(slider) {
						var target = slider.animatingTo;
						var current = slider.find('.flex-active').parent().index();
						var $items = slider.find('.titanium_zooming_slider_item').removeClass('active');
						$items.eq(target+1).add((target==0) ? $items.last() : (target==$items.length-3 ? $items.first() : '')).addClass('active');
						if (target==0) {
							$('.titanium_zooming_slider .slides li:first-child').next().next().clone().appendTo('.titanium_zooming_slider .slides').removeClass('clone').addClass('clone_end');
						}
						else if (target==$items.length-3) {
							$('.titanium_zooming_slider .slides li:last-child').prev().prev().clone().appendTo('.titanium_zooming_slider .slides').removeClass('clone').addClass('clone_start');
						}
						slider.find('.titanium_zooming_slider_ghost.shown').css('left',(target-current)*100 + 50 + '%').removeClass('shown');
						slider.find('.flex-control-nav li').eq(target).find('.titanium_zooming_slider_ghost').addClass('shown');
					},
					after: function(slider) {
						slider.find('.clone_start, .clone_end').remove();
						slider.find('.flex-control-nav li .titanium_zooming_slider_ghost').css('left','');
					}
				});
			});
		}
	...
	function initMasonryGrid(){
	    ...
	    $('.titanium_masonrygrid_item').hover(function(){
	    	$(this).find('.titanium_masonrygrid_item_overlay').stop().animate({'opacity' : 1}, 140);
	    }, function(){
	    	$(this).find('.titanium_masonrygrid_item_overlay').stop().animate({'opacity' : 0}, 140);
	    });

	    $('.titanium_masonrygrid_item').css({visibility: "visible", opacity: "0"});

	    if($('.titanium_masonrygrid').length){

	        $('.titanium_masonrygrid').each(function(){
	            var $this = $(this);
	            $this.imagesLoaded(function(){
	                $this.animate({opacity:1});
	                $this.isotope({
	                    itemSelector: '.titanium_masonrygrid_item',
	                    masonry: {
	        ...
	        // Fade In
		    $('.titanium_masonrygrid_item').each(function(index){
		        $(this).delay(80*index).animate({opacity: 1}, 200);
		    });

	        $(window).resize(function(){
	            resizeMasonryGrid($('.grid-sizer').width());
	            $('.titanium_masonrygrid').isotope('reloadItems');
	        });
	...
	function resizeMasonryGrid(size){
		...
		var square_small = $('.masonry_ss .titanium_masonrygrid_item_wrap');
		var square_big = $('.masonry_sb .titanium_masonrygrid_item_wrap');
	    var rectangle_portrait = $('.masonry_rp .titanium_masonrygrid_item_wrap');
	    var rectangle_landscape = $('.masonry_rl .titanium_masonrygrid_item_wrap');
